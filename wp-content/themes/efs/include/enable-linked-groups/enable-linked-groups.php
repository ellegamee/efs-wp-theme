...
function enable_linked_groups_enqueue_block_editor_assets() {
	// Define the base URL for assets within the theme.
	$base_asset_url = get_stylesheet_directory_uri() . '/include/enable-linked-groups/build/';
	// Define the base path for assets within the theme.
	$base_asset_path = get_stylesheet_directory() . '/include/enable-linked-groups/build/';

	$asset_file  = include $base_asset_path . 'index.asset.php';

	wp_enqueue_script(
		'enable-linked-groups-editor-scripts',
		$base_asset_url . 'index.js',
	...
	wp_set_script_translations(
		...
		get_stylesheet_directory() . '/include/enable-linked-groups/languages'
	);

	wp_enqueue_style(
		'enable-linked-groups-editor-styles',
		$base_asset_url . 'editor.css'
	);
}
...
function enable_linked_groups_block_styles() {
	wp_enqueue_block_style(
		...
		array(
			...
			'src'    => get_stylesheet_directory_uri() . '/include/enable-linked-groups/build/style.css',
			'ver'    => wp_get_theme()->get( 'Version' ),
			'path'   => get_stylesheet_directory() . '/include/enable-linked-groups/build/style.css',
		)
	);
}
